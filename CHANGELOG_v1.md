...
This release corrects a publishing error with the `@gseller/admin-ui` packages. There are no code changes in this release.
...
* The upgrade of the Admin UI to Angular v10 means that if you are using the `@gseller/ui-devkit` package to compile an extended version of the Admin UI, you need to have at least TypeScript v3.9.2 installed.
...
* The `ID` type in `@gseller/common/lib/generated-types` & `@gseller/common/lib/generated-shop-types` is now correctly typed as `string | number`, whereas previously it was `string`. If you are using any generated types in your plugin code, this may lead to TypeScript compiler errors which will need to be corrected.
...
* The `@gseller/admin-ui-plugin` now contains only the default admin ui app.
* To create extensions, you will need to install `@gseller/ui-devkit`, which exposes a `compileUiExtensions()` function.
...
* The `LocalAssetStorageStrategy` class has been removed from `@gseller/core` and now lives in the `@gseller/asset-server-plugin` package.
...
*Note: only the `@gseller/email-plugin` package was updated in this release, as v0.6.2 included a critical bug which prevented installation via `@gseller/create`*
...
**Note** only the `@gseller/admin-ui`, `@gseller/admin-ui-plugin` & `@gseller/email-plugin` packages were updated in this release.
...
* **admin-ui** Expose public API at @gseller/admin-ui/devkit ([c2742ec](https://github.com/vendure-ecommerce/vendure/commit/c2742ec))
